...
foreach ($endereco->findAll() as $key => $value) {
    ...
    $fields = array($value->cep, $value->endereco, $value->cnpj,
        $value->numero, "<a href='editarEndereco.php?acao=editar&id=" . $value->id . "'><input type='submit' name='editar' class='btn btn-primary' value='Editar' style='margin-right:30px;margin-left:50px;'></a><a href='inativarEndereco.php?acao=deletar&id=" . $value->id . "' onclick='return confirm(\"Deseja realmente deletar?\")'><input type='submit' name='deletar' class='btn btn-danger' value='Deletar' style='margin-right:-100px;'></a>");
